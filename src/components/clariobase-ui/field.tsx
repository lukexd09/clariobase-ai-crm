...
export function Label({ className = "", ...props }: ComponentProps<"label">) {
  return <label data-slot="label" className={cn("text-sm font-medium text-[color:var(--cb-foreground)]", className)} {...props} />;
}
...
export function FormMessage({ className = "", ...props }: ComponentProps<"p">) {
  return <p data-slot="form-message" className={cn("text-sm leading-6 text-[color:var(--cb-danger)]", className)} {...props} />;
}
