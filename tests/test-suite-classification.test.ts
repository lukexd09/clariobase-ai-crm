...
test("every test file belongs to exactly one fast or infrastructure suite", () => {
  ...
  assert.ok(fastTests.includes("tests/e2e-guard.test.ts"));
  assert.ok(fastTests.includes("tests/e2e-commands.test.ts"));
  assert.ok(fastTests.includes("tests/e2e-fixture-negative.test.ts"));
