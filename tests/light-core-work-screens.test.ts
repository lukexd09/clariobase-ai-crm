...
test("core CRM work screens use the light shell baseline", () => {
  ...
  assert.match(appShell, /focus-visible:ring-2/);
...
test("core CRM work screens keep semantic light status pills and accessible tables", () => {
  ...
  assert.match(workPage, /inline-flex min-h-9 shrink-0 items-center justify-center whitespace-nowrap rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium/);
  ...
  assert.match(appShell, /focus-visible:ring-2/);
  assert.match(homepage, /Daily CRM workspace/);
});
...
test("sales report header stays compact and KPI emphasis is conditional", () => {
  ...
  assert.match(salesReportPage, /text-xs font-semibold uppercase tracking-\[0\.18em\] text-sky-700/);
